...
while True:
    ...
    if inputSistema == 0:
        ...
        print(f"Sistema Operacional    Votos    %")
        print("---------------------  ------   ----")
        print(f"Windows Sever           {votos_por_sistema.keys()}       {votos_por_sistema.keys('Windows')}")
        print(f"Unix                    {votos_por_sistema.keys()}       {votos_por_sistema.keys('Unix')}")
        print(f"Linux                   {votos_por_sistema.keys()}       {votos_por_sistema.keys('Linux')}")
        print(f"Netware                 {votos_por_sistema.keys()}       {votos_por_sistema.keys('Netware')}")
        print(f"Mac_Os                  {votos_por_sistema.keys()}       {votos_por_sistema.keys('MacOs')}")
        print(f"Outro                   {votos_por_sistema.keys()}       {votos_por_sistema.keys('Outros')}")
        print("---------------------  ------   ----")
        print(f"Total de Votos:         ")
        break

    elif inputSistema > 6 or inputSistema < 0:   
        print("Valor inválido")
        


def ehMaiorVoto(nome_so):
    return votos_por_sistema.get(nome_so) == max(votos_por_sistema.keys())
